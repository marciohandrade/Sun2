...
// 캐시조회
	...
	// 캐시조회 : SUN_CN, SUN_RU, SUN_JP, BT_TH, BT_CN, MU_JP(GameChu, GameOn), MU_VN, MU_TW
	bool InquireCash(
		long	GameCode,
...
// 상품구매
	...
	// 상품구매 : SUN_CN, SUN_RU, SUN_JP, MU_JP(GameChu, GameOn), MU_VN, MU_TW
	bool BuyProduct(
		long	GameCode,
		ShopProtocol::PaymentType	PaymentType,
		DWORD	AccountSeq, 
	...
	bool BuyProduct(
		...
		DWORD	dwIPAddress, 
		bool	DeductMileageFlag,
		double	ItemUniqueID);						// OnBuyProduct로 돌려줌
...
// 상품선물
	...
	// 상품선물 : SUN_CN, SUN_RU, SUN_JP, MU_JP(GameChu, GameOn), MU_VN, MU_TW
	bool GiftProduct(
		long	GameCode,
		ShopProtocol::PaymentType	PaymentType,
		DWORD	AccountSeq, 
	...
	bool GiftProduct(
		...
		DWORD	SenderAccountSeq,
		int		USN,							// BT_CN : tencent qq 계정번호
		WCHAR	SenderAccountID[MAX_ACCOUNTID_LENGTH],
		WCHAR	SenderCharName[MAX_CHARACTERID_LENGTH], 
		long	SenderServerIndex, 
		DWORD	ReceiverAccountSeq, 
		WCHAR	ReceiverAccountID[MAX_ACCOUNTID_LENGTH],
		WCHAR	ReceiverCharName[MAX_CHARACTERID_LENGTH], 
		long	ReceiverServerIndex,
		long	ReceiverClass,					// <-- 보관함 사용을 처리해주기 위해 필요
		long	ReceiverLevel,					// <-- 보관함 사용을 처리해주기 위해 필요
		WCHAR	ReceiverRank[MAX_RANK_LENGTH],	// <-- 보관함 사용을 처리해주기 위해 필요
		WCHAR	Message[MAX_MESSAGE_LENGTH], 
		long	PackageProductSeq, 
		long	PriceSeq, 
		long	SalesZone, 
...
// 캐시차감
	// 해외 빌링시스템 연동시 차감만을 호출 : BT_TH
	bool DeductCashOutbound(
		long	GameCode,							// 게임코드
		ShopProtocol::PaymentType	PaymentType,	// 결제 타입
		DWORD	TransactionID,						// 트랜잭션 ID : 비동기 통신을 위함
		DWORD	AccountSeq,							// 계정순번
		WCHAR	AccountID[MAX_ACCOUNTID_LENGTH],	// 계정ID
		WCHAR	CharName[MAX_CHARNAME_LENGTH],		// 캐릭터 명
		DWORD	AccessIP,							// 클라이언트IP
		DWORD	DeductPrice,						// 차감할 가격(이것이 핵심!)
		DWORD	ItemID,								// 아이템 ID
		DWORD	ItemType,							// 아이템 Type
		WCHAR	ItemName[MAX_PRODUCTNAME_LENGTH],	// 아이템 이름
		DWORD	Period,								// 주기 : 받기만 하고 결과로 돌려줌
		double	ItemUniqueID);						// 아이템 유니크 ID : 받기만 하고 결과로 돌려줌

	// 해외 빌링시스템 연동시 차감에 대한 롤백 : BT_TH
	bool RollbackDeductCashOutbound(
		long	GameCode,							// 게임코드
		ShopProtocol::PaymentType	PaymentType,	// 결제 타입
		DWORD	TransactionID,						// 트랜잭션 ID : 비동기 통신을 위함
		DWORD	AccountSeq,							// 계정순번
		INT64	DeductNo);							// OnDeductCashOutbound()에서 리턴받은 차감번호(DeductNo)

	// 해외 빌링시스템 연동시 차감만을 호출 : BT_CN
	// 지정된 TimeOut 안에 CommitOrRollbackDeductCashOutbound() 의 IsCommit에 true로 호출해줘야 함
	bool DeductCashOutbound(
		long	GameCode,							// 게임코드
		ShopProtocol::PaymentType	PaymentType,	// 결제 타입
		DWORD	TransactionID,						// 트랜잭션 ID : 비동기 통신을 위함
		...
		int		USN,								// BT_CN qq계정
		DWORD	AccessIP,							// 클라이언트IP
		DWORD	DeductPrice,						// 차감할 가격
		DWORD	ItemID,								// 아이템 ID
		DWORD	Period,								// 주기 : 받기만 하고 결과로 돌려줌
		double	ItemUniqueID,						// 아이템 유니크 ID : 받기만 하고 결과로 돌려줌
		DWORD	Timeout = 5000);					// 타임아웃. 타임아웃 전에 commit 혹은 rollback 하지 않으면 자동 rollback 됨. [msec]

	// 해외 빌링시스템 연동시 차감에 대한 커밋 : BT_CN
	bool CommitDeductCashOutbound(
		long	GameCode,							// 게임코드
		ShopProtocol::PaymentType	PaymentType,	// 결제 타입
		DWORD	TransactionID,						// 트랜잭션 ID : 비동기 통신을 위함
		DWORD	AccountSeq,							// 계정순번
		...
		int		USN,								// BT_CN qq계정
		DWORD	DeductPrice,						// 차감할 가격
		CHAR	DeductNo[MAX_DEDUCTNO_LENGTH]);		// BT_CN용 OnDeductCashOutbound()에서 리턴받은 차감번호(DeductNo)
	// 해외 빌링시스템 연동시 차감에 대한 롤백 : BT_CN
	bool RollbackDeductCashOutbound(
		long	GameCode,							// 게임코드
		ShopProtocol::PaymentType	PaymentType,	// 결제 타입
		DWORD	TransactionID,						// 트랜잭션 ID : 비동기 통신을 위함
...
// 캐시조회 결과
	...
	// 캐시조회 결과 : SUN_CN, SUN_RU, SUN_JP, BT_TH, MU_JP(GameChu, GameOn), MU_TH, MU_TW, MU_VN
	virtual void OnInquireCash(
		DWORD	AccountSeq,
		long	ResultCode,
...
// 상품구매 결과
	...
	// 상품구매 결과 : SUN_CN, SUN_RU, SUN_JP, MU_JP(GameChu, GameOn), MU_TH, MU_TW, MU_VN
	virtual void OnBuyProduct(
		DWORD	AccountSeq,
		long	ResultCode,
		long	OutBoundResultCode,
	...
	virtual void OnBuyProduct(
		...
		long	OutBoundResultCode,
		long	LeftProductCount,
		double	CashSum,					// <-- 추가 : 잔여 캐시
		double	DeductedPrice,				// <-- 추가 : 차감된 캐시
		double	ItemUniqueID,				// <-- 추가 : bypass
		BYTE	PropertyCount,		
		ShopProtocol::STItemPropertyAll PropertyList[]);
...
// 상품선물 결과
	...
	// 상품선물 결과 : SUN_CN, SUN_RU, SUN_JP, MU_JP(GameChu, GameOn), MU_TH, MU_TW, MU_VN
	virtual void OnGiftProduct(
		DWORD	SenderAccountSeq,
		long	ResultCode,
		long	OutBoundResultCode,
	...
	// 상품선물 결과 : BT_TH, BT_CN
	virtual void OnGiftProduct(
		DWORD	SenderAccountSeq,							// 발신자 계정순번
		WCHAR	SenderAccountID[MAX_ACCOUNTID_LENGTH],		// 발신자 아이디
		WCHAR	SenderCharName[MAX_CHARACTERID_LENGTH],		// 발신자 캐릭터 명
		DWORD	ReceiverAccountSeq,							// 수신자 계정순번
		WCHAR	ReceiverAccountID[MAX_ACCOUNTID_LENGTH],	// 수신자 아이디
		WCHAR	ReceiverCharName[MAX_CHARACTERID_LENGTH],	// 수신자 캐릭터 명
		WCHAR	SendMessage[MAX_MESSAGE_LENGTH],			// 선물 메시지
		double	CashSum,									// 잔여 캐시
		double	DeductedPrice,								// 차감된 캐시
		long	ResultCode,									// 결과 코드
		long	OutBoundResultCode,							// OB 결과 코드 : ResultCode -3 인 경우 체크
		long	LeftProductCount,							// 상품 판매 가능 잔여수량 (-1: 무제한) 상품 개수 정보를 갱신해야한다.
		BYTE	PropertyCount,								// 속성(PropertyList) 개수
		ShopProtocol::STItemPropertyAll PropertyList[]);	// 속성 리스트
...
// 공통 서비스
	...
	// 아웃바운드 캐시 차감 : BT_TH
	virtual void OnDeductCashOutbound(
		long	ResultCode,						// 결과코드
		long	OutBoundResultCode,				// Outbound 상세에러
		DWORD	TransactionID,					// async 통신용 트랜잭션 id
		DWORD	AccountSeq,						// DeductCashOutbound()에서 넘겨준 값
		WCHAR	AccountID[MAX_ACCOUNTID_LENGTH],// DeductCashOutbound()에서 넘겨준 값
		DWORD	DeductedPrice,					// 차감된 금액
		INT64	DeductNo,						// 차감성공시 차감번호
		double	CashSum,						// 잔여캐시
		DWORD	Period,							// 주기 : 받기만 하고 결과로 돌려줌
		double	ItemUniqueID);					// 아이템 유니크 ID : 받기만 하고 결과로 돌려줌

	// 아웃바운드 캐시 차감 롤백 : BT_TH
	virtual void OnRollbackDeductCashOutbound(
		long	ResultCode,						// 결과코드
		long	OutBoundResultCode,				// Outbound 상세에러
		DWORD	TransactionID,					// async 통신용 트랜잭션 id
		DWORD	AccountSeq,						// DeductCashOutbound()에서 넘겨준 값
	...
	virtual void OnDeductCashOutbound(
		...
		WCHAR	AccountID[MAX_ACCOUNTID_LENGTH],// DeductCashOutbound()에서 넘겨준 값
		DWORD	DeductedPrice,					// 차감된 금액
		CHAR	DeductNo[MAX_DEDUCTNO_LENGTH],	// 차감번호
		double	CashSum,						// 잔여캐시
		DWORD	Period,							// 주기 : 받기만 하고 결과로 돌려줌
		double	ItemUniqueID);					// 아이템 유니크 ID : 받기만 하고 결과로 돌려줌

	// 아웃바운드 캐시 차감 커밋 : BT_CN
	virtual void OnCommitDeductCashOutbound(
		long	ResultCode,						// 결과코드
		long	OutBoundResultCode,				// Outbound 상세에러
		DWORD	TransactionID,					// async 통신용 트랜잭션 id
		DWORD	AccountSeq,						// DeductCashOutbound()에서 넘겨준 값
		DWORD	DeductPrice,					// 차감 금액
		double	CashSum,						// 잔여캐시
		CHAR	DeductNo[MAX_DEDUCTNO_LENGTH]);	// 차감번호

	// 아웃바운드 캐시 차감 롤백 : BT_CN
	virtual void OnRollbackDeductCashOutbound(
		long	ResultCode,						// 결과코드
		long	OutBoundResultCode,				// Outbound 상세에러
		DWORD	TransactionID,					// async 통신용 트랜잭션 id
