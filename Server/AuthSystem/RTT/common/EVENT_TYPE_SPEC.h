...
// 게임함수변조
#define AKCHECK_APIHOOK					0x00000008

// 아머킷 변조
#define AKCHECK_AKINTEGRITY				0x00000010

// 디버그 상태탐지
#define AKCHECK_DEBUG					0x00000020

// 메모리조작
#define AKCHECK_MEMEDIT					0x00000040

// 의심가는 S/W 탐지
#define AKCHECK_DOUBT					0x00000080

// 런타임 코드 변조를 탐지
#define AKCHECK_RUNCODE					0x00000100
...
// 디폴트 런쳐
#define DEFAULT_PARENT	"EXPLORER.EXE"

// 디폴트 SUN 런쳐
#define SUN_LAUNCHER	"SUN.exe"

#define	MAX_HASHSTRING	256

/*-----------------------------------------------------------------------------
 * 에러 코드 정의
**---------------------------------------------------------------------------*/
#define AKERROR_INIT_FAIL					-1		// 초기화가 되지 않았음
#define AKERROR_GENERAL_APIFAIL				-2		// Windows API함수 호출이 실패했음
#define AKERROR_INIT_INVALID_CALLBACKPROC	-10		// 초기화중에 사용자가 콜백함수를 정의하지 않았음
#define	AKERROR_SET_PRIVILEGE				-11		// 프로세스를 스캔하기 위해 권한 상승에 실패하였음, 
													// 관리자권한으로 프로그램을 실행시키는 것이 요구됨
#define AKERROR_TIMER_SETFAIL				-12		// 초기화중에 타이머를 세팅하지 못했음
#define AKERROR_INIT_INVALID_HANDLE			-13		// 초기화변수의 HWND 가 유효하지 않음
#define AKERROR_PECHKSUM_ZERO				-14		// PE헤더의 체크섬이 0입니다. 
#define	AKERROR_CONCHKSUM_FAIL				-15		// 첵섬값을 만드는데 실패했습니다.
#define AKERROR_FILEFROMID_GETFAIL			-16		// ID로 부터 화일을 가져오는데 실패했습니다.
#define AKERROR_CHKHOOK_FAIL				-17		// API Hook 체크에 실패했습니다. API들을 신뢰할 수 없습니다.
#define AKERROR_MD5HASH_MAKEFAIL			-18		// 화일의 checksum값을 md5hash로 만드는데 실패했습니다.
#define AKERROR_VERSION_NOTMATCHED			-19		// 버전이 일치하지 않습니다.
...
typedef struct _PROCESS_SCAN_INFO
{
	...
	DWORD	SianatureType;
	TCHAR	SignatureName[MAX_SIG_NAME + 1];
...
// isDebuggerPresent() 함수의 프로토타입
typedef BOOL	(* LPFNLOCALDEBUG ) ( void ) ;

// CheckRemoteDebuggerPresent() 함수의 프로토타입
typedef	BOOL	(* LPFNREMOTEDEBUG ) ( HANDLE, PBOOL ) ;

// 디버거 체크 함수
INT		CheckDebugger(IN HANDLE hTargetProcess, TCHAR* lptszLauncherName, OUT PANTI_DEBUG_CHECK_INFO pADCI ) ;

// 부모 프로세스의 이름을 얻어오는 함수
INT		GetParentProcessName (CHAR *parent_name, INT max_len);

/*-----------------------------------------------------------------------------
 * md5hash 헤더
**---------------------------------------------------------------------------*/
VOID	ReleaseHash( BYTE* pHashData ) ;
BYTE*	ConstructChecksumToHash( IN DWORD dwCheckSum, OUT LPDWORD pdwSize ) ;
INT		ConstructMD5HashFromChecksumInFile ( IN TCHAR* lptszFileNameOUT, OUT PPE_HASH_INFO pPHI);
BYTE*	MakeMemToHash(IN DWORD dwMemAddr, IN DWORD dwMemSize, OUT LPDWORD pdwSize ) ;
 ...
 * authmem 헤더
**---------------------------------------------------------------------------*/
INT		AuthRuntimeCodeSection( DWORD	dwPid ) ;
 ...
 * authpe 헤더
**---------------------------------------------------------------------------*/
#define	AKREGKEY_PEDIGEST	"SOFTWARE\\WebZen\\Sun\\Digest" 
#define AKREGKEY_BINNAME	"Binary"

#define AKDIGEST_DATA1_32	0x01030507
#define AKDIGEST_DATA2_32	0x0b111316

INT		AuthPEChecksum  ( TCHAR* lptszFileName, PPE_CHECKSUM_INFO pPCI ) ;
INT		AuthPEIntegrity ( TCHAR* lptszFileName , DWORD dwType, LPVOID lptagPEAuth ) ;
DWORD	GetPEHeaderChecksum   (  TCHAR * lptszFileName ) ;
DWORD	GetPECalculatedChecksum (  TCHAR * lptszFileName ) ;
...
// Malicious S/W를 탐지하기 위한 탐지시그니쳐 ( file based )
typedef struct _SIGNATURE 
{
	LPTSTR	name;					// 탐지 s/w 명명 
	int		type ;					// 탐지대상의 종류 (타입)
	int		f_offset;				// 화일 오프셋 
	int		siglen;					// signature의 byte	
	char	*signature ;			// 탐지대상 s/w 의 signature 
} SIGNATURE ;
...
typedef struct _PROC_SIGNATURE
{
	...
	TCHAR	lptszBinName[256];			// malicious s/w name
	TCHAR	lptszSigName[256];			// signature name
	DWORD 	dwOffset;				// 프로세스상에서의 signature 의 offset (from Image base address)
	DWORD 	dwSigLen;				// lpszSignature length
	TCHAR	lpszSignature[512];			// 실제 signature 	
	TCHAR	lptszDescription[256];		// 설명...실제 코드에는 NULL로 처리 
...
typedef BOOL (*lpfnSysDllAddressCB) (PADDR_RANGE pAddrRange, PVOID pTag);
...
/**	-----------------------------------------------------------------------
	\brief	AntiHook 모듈 초기화 함수
-------------------------------------------------------------------------*/
INT	InitAntiHook(VOID);

/**	-----------------------------------------------------------------------
	\brief	AntiHook 모듈 종료 함수
-------------------------------------------------------------------------*/
VOID TerminateAntiHook(VOID);

/**	-----------------------------------------------------------------------
	\brief	Windows API 후킹 탐지
-------------------------------------------------------------------------*/
INT CheckWinApiHook(IN PHOOKED_API_INFO pHAI);
					
/**	-----------------------------------------------------------------------
	\brief	주요 시스템 DLL 들의 주소값 리스트를 리턴		
-------------------------------------------------------------------------*/
INT GetSystemDllsAddress(lpfnSysDllAddressCB lpSysDllAddrCB, PVOID pTag);
