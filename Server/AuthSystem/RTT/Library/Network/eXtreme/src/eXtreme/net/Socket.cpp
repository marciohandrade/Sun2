...
int Socket::Send( char* buffer, int length, AsyncResult* result )
{
	WSABUF	wsaBuf;
	wsaBuf.buf	= buffer;
	wsaBuf.len	= length;
...
int Socket::Recv( char* buffer, int bufSize, AsyncResult* result )
{
	WSABUF	wsaBuf;
	wsaBuf.buf	= buffer;
	wsaBuf.len	= bufSize;
...
HANDLE Socket::GetNativeHandle()
{
	...
	return (m_socket == INVALID_SOCKET ) ? INVALID_HANDLE_VALUE : (HANDLE)m_socket;
}
