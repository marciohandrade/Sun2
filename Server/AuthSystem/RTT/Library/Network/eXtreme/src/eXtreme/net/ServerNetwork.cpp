...
bool ServerNetwork::Create( WORD sessionCnt, SessionManager* sm, DWORD threadCnt )
{
...
bool ServerNetwork::OpenAcceptor( char* ip, u_short portNo )
{
	return Acceptor::Open( ip, portNo );
}

void ServerNetwork::CloseAcceptor()
{
	Acceptor::Close();
}

void ServerNetwork::OnAccept( SOCKET s, Socket::SocketAddr& addr )
{
	ServerSession* session = m_sessionManager->BindSession( s, &addr );
	if( session == 0 )
	{
		::closesocket( s );
		return;
...
unsigned int __stdcall ServerNetwork::Dispatcher( LPVOID parameter )
{
	...
	while( 1 )
	{
		...
		if( handler == 0 )
			break;
//		{
//			CallbackErrorHandler( ::WSAGetLastError(), "[eXtreme::ServerNetwork::Dispatcher] Handler is NULL" );
//			break;
//		}

		handler->HandleEvent( result );

		//if( result->eventType == Event_Accept )
		//{
		//	instance->m_sessionManager->ActiveSession( ( ServerSession* )handler );
		//}
		//else 
		...
		{
			instance->m_sessionManager->InactiveSession( ( ServerSession* )handler );
		}
	}

	CallbackErrorHandler( 0, "[eXtreme::ServerNetwork::Dispatcher] Dispatcher thread is End" );
...
void ServerNetwork::ConnectSession( ServerSession* s, char* ipAddress, unsigned short portNo )
{
	m_connector.Connect( s, ipAddress, portNo );
}

bool ServerNetwork::AddSessionEvent( ServerSession* s )
{
	return m_eventQueue.AddEvent( s );
}

void ServerNetwork::PostCompletion( EventHandler* handler, AsyncResult* result )
{
