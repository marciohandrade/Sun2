...
OnServiceStart()
{
    ...
    LoginFrontFrame* const pLoginFrontFrame = LoginFrontFrame::Instance();
    return (TRUE == pLoginFrontFrame->DoInit("./LoginFront.ini",
                                             LoginFrontFrame::DoAllocServerSession,
                                             LoginFrontFrame::DoFreeServerSession,
                                             LoginFrontFrame::DoFreeConnectSession,
                                             LoginFrontFrame::DoAllocClient,
                                             LoginFrontFrame::DoFreeClient,
                                             LoginFrontFrame::DoFreeConnect_NotUsed));
}

void
OnServiceProcess()
{
    LoginFrontFrame::Instance()->DoProcess();
}

bool
OnServiceUpdate()
{
    return (TRUE == LoginFrontFrame::Instance()->DoUpdate());
}

void
OnServiceStop()
{
    LoginFrontFrame::Instance()->DoRelease();
    LoginFrontFrame::DestroyInstance();
...
//
//extern SolarLog * g_pLoger[LOG_MAX];
int
