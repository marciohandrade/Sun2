...
/********************************************************************
	created:	2010/11/23
	created:	23:11:2010   11:55
	filename: 	d:\Svn\trunk\Server\Common\Pms\IPMSObject.h
	file path:	d:\Svn\trunk\Server\Common\Pms
	file base:	IPMSObject
	file ext:	h
	author:		youngmoon
	
	purpose:	PMS 서버에서 메시지를 전송 받을때 호출 되는 순수 가상 함수들 정의
*********************************************************************/
...
interface IPMSRegionInfoList
{
    ...
    virtual void AddRegionInfo272(DWORD dwSSN, DWORD dwCategoryID, LONG pvRegionStat[238]) = 0;
};
...
public:
	...
	*/
    ...
    /*
    To send the statistics information for users to PMS Administrator ( in the case of internet cafe )
    @param1 : The pointer of IPMSRegionInfoListPC
    @return value : If @param1 has the statistics information, return TRUE
    else return FALSE
    */
    virtual BOOL OnRegionInfoPCReq(IPMSRegionInfoListPC* plstRegionInfo) = 0;
    /*
    To send the statistics information for users to PMS Administrator ( in the case of internet cafe )
    @param1 : The pointer of IPMSStatInfoListPC
    @return value : If @param1 has the statistics information, return TRUE
    else return FALSE
    */
