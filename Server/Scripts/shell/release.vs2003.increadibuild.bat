...
set DEVENV_PATH=C:\Program Files\Microsoft Visual Studio 9.0\VC\bin
...
set LOGLIB=
rem use IncrediBuild : "--use-incredibuild" or "-i"
set INCREDIBUILD=--use-incredibuild
...
%RUBY_PATH%\ruby.exe release.rb -u %USERNAME% -p %PASSWORD% %QUIET% %NO_UPDATE% %BUILD_GAME% %BUILD_AUTH% %REBUILD% %GAMELIB% %LOGLIB% -c "%P4_ROOT_PATH%" -t "%RELEASE_ROOT%" "%REPO_URL%" "%INCREDIBUILD%"
