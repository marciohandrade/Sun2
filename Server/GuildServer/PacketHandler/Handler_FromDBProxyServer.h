...
private:
    ...
    Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_DUTY_ACK);		// 길드 직책
	...
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_MASTER_ACK);		// 길드 마스터 변경
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_MASTER_NAK);

	Handler_FromDBProxyServer_DECL(DZ_GUILD_CREATE_CORPS_ACK);			// 길드 군단 생성
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CREATE_CORPS_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_DESTROY_CORPS_ACK);			// 길드 군단 해산
	Handler_FromDBProxyServer_DECL(DZ_GUILD_DESTROY_CORPS_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_COMMANDER_ACK);		// 길드 군단장 변경
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_COMMANDER_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_CORPS_ACK);			// 길드 군단 이동
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CHANGE_CORPS_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CREATE_REAR_CAMP_ACK);		// 길드 훈련캠프 창설
	Handler_FromDBProxyServer_DECL(DZ_GUILD_CREATE_REAR_CAMP_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_DESTROY_REAR_CAMP_ACK);		// 길드 훈련캠프 해산
	Handler_FromDBProxyServer_DECL(DZ_GUILD_DESTROY_REAR_CAMP_NAK);
	Handler_FromDBProxyServer_DECL(DZ_GUILD_LOGOUT_CHAR_CMD);			// 길드멤버 로그아웃
	Handler_FromDBProxyServer_DECL(DZ_GUILD_GM_GUILD_POINT_ACK);		// GM명령어 길드포인트
	Handler_FromDBProxyServer_DECL(DZ_GUILD_GM_GUILD_POINT_NAK);		// GM명령어 길드포인트
	Handler_FromDBProxyServer_DECL(DZ_GUILD_SELECT_CORPS_NAK);			// 군단 정보 요청 실패.

	Handler_FromDBProxyServer_DECL(DZ_GUILD_RELATION_ACK);			// 길드 관계
