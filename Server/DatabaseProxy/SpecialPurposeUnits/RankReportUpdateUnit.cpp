    ...
    , completed_update_query_(false)
{
    ...
    ZeroMemory(&rank_info_, sizeof(rank_info_));
}
...
RankReportUpdateUnit::SelectedItemList(const KeyNode& index, QueryResult* query_result_base)
{
    ...
    if (number_of_rows == 0)
    {
        MakeUpdateQuery(0, false);
        MakeUpdateQuery(0, true);
        rank_info_.guid = 0;
    ...
    for (; row < number_of_rows; ++row)
    {
        ...
        if (rank_info_.guid == 0) {
            rank_info_.guid = result.guid;
        };
        bool update_current_guid = (rank_info_.guid != result.guid);
        if (update_current_guid)
        {
            // ACTION: insert query & refresh
            MakeUpdateQuery(result.guid, false);
            MakeUpdateQuery(result.guid, true);
        }
        rank_info_.guid = result.guid;
        // update result information
        if (item_info->IsAccessory() || item_info->IsSpecialAccessory()) {
            ++rank_info_.accessory_ranks[convert_result_rank];
        }
        else {
            ++rank_info_.ranks[convert_result_rank];
        }
        //
    }
...
bool RankReportUpdateUnit::MakeUpdateQuery(int new_guid, bool is_accessory)
{
    __UNUSED(new_guid);
    ;{
        const short* it = is_accessory ?
            &rank_info_.accessory_ranks[RANK_D] :
            &rank_info_.ranks[RANK_D];

        const short* end = is_accessory ?
            &rank_info_.accessory_ranks[_countof(rank_info_.accessory_ranks)] :
            &rank_info_.ranks[_countof(rank_info_.ranks)];
    ...
    short* src_ptr = is_accessory ? rank_info_.accessory_ranks : rank_info_.ranks;
    Query_SetRankReport* query = update_node->query_;
    if (Query_SetRankReport::sQUERY_PARAM* parameter = &query->parameter_[0]) {
        parameter->guid = rank_info_.guid;
        BOOST_STATIC_ASSERT(sizeof(parameter->rank_array[0]) == sizeof(rank_info_.ranks[0]) &&
                            sizeof(parameter->rank_array) == sizeof(rank_info_.ranks) &&
                            sizeof(rank_info_.ranks) == sizeof(rank_info_.accessory_ranks));
        CopyMemory(parameter->rank_array, src_ptr, sizeof(parameter->rank_array));
        parameter->rank_array[0] = is_accessory;
    }
    //
    util::LList::AddPrev(update_node, &arguments_.pending_nodes_);
    ++arguments_.number_of_pending_nodes_;
    // clear accumulators
    ZeroMemory(src_ptr, sizeof(rank_info_.ranks));
