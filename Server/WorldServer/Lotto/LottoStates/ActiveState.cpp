...
void LottoActiveState::OnInfoRequest(CHARGUID charGuid, MSG_BASE* packet)
{
...
void LottoActiveState::OnEntryRequest(CHARGUID charGuid, MSG_BASE* packet)
{
    MSG_CW_LOTTO_ENTRY_SYN* pRecvMsg = static_cast<MSG_CW_LOTTO_ENTRY_SYN*>(packet);

    int drawNum       = pRecvMsg->wDrawNum;
    bool isAuto       = (pRecvMsg->bAuto != 0);
    BYTE* pickedNum   = pRecvMsg->bPickedNum;
    ...
    if (result) {
        ...
        if (result) {
            RequestEntrySave(charGuid, pRecvMsg->bAuto, (BYTE)status, pickedNum);
        }
    }
#else
...
{
    MSG_AW_USER_LOTTO_TICKET_PURCHASE_ACK* pRecvMsg = \
        static_cast<MSG_AW_USER_LOTTO_TICKET_PURCHASE_ACK*>(packet);

    int result_code = pRecvMsg->iResult;
    CHARGUID charGuid = pRecvMsg->iCharGuid;
    int drawNum = pRecvMsg->wDrawNum;
    bool isAuto = (pRecvMsg->bAuto != 0);
    BYTE* pickedNum = pRecvMsg->bPickedNum;
...
void LottoActiveState::OnEntrySave(MSG_BASE* packet)
{
    MSG_DW_LOTTO_SAVE_ENTRY_ACK* pRecvMsg = \
        static_cast<MSG_DW_LOTTO_SAVE_ENTRY_ACK*>(packet);

    CHARGUID charGuid = pRecvMsg->iCharGuid;
    int drawNum = pRecvMsg->wDrawNum;
    int status = pRecvMsg->bStatus;
    BYTE* pickedNum = pRecvMsg->bPickedNum;
    ...
    if (pRecvMsg->iResult != 0) {
        ...
        SendToClient(charGuid, (BYTE*)&nak, (WORD)sizeof(MSG_CW_LOTTO_ENTRY_NAK));
        return;
    }
...
#endif
    SendToClient(charGuid, (BYTE*)&ack, (WORD)sizeof(ack));
}

void LottoActiveState::OnTicketAcquisition(MSG_BASE* packet)
{
    MSG_DW_LOTTO_ADD_TICKET_ACK* pRecvMsg = \
        static_cast<MSG_DW_LOTTO_ADD_TICKET_ACK*>(packet);
    ...
    int numRemainingTickets = pRecvMsg->bNumRemainingTickets;
...
void LottoActiveState::OnHeimConsumption(MSG_BASE* packet)
{
    MSG_AW_USER_STATUS_HEIM_CONSUMPTION_NTF* pRecvMsg = \
        static_cast<MSG_AW_USER_STATUS_HEIM_CONSUMPTION_NTF*>(packet);

    CHARGUID charGuid = pRecvMsg->m_CharGuid;
    eHEIM_CONSUMPTION_TYPE type = (eHEIM_CONSUMPTION_TYPE)pRecvMsg->m_byConsumptionType;
    INT64 amount = (INT64)pRecvMsg->m_Amount;
    int percentage = -1;
    //_NA_0_20091119_HEIM_LOTTO_ACCUM_CTRL_BY_SHOP
    percentage = pRecvMsg->m_LottoAccumRatio;
...
void LottoActiveState::BroadcastTotalAccum(INT64 totalAccum, bool dbWrite)
{
    ...
    BroadcastToClients((BYTE*)&sendMsg, (WORD)sizeof(MSG_CW_LOTTO_ACCUM_BRD));
}
