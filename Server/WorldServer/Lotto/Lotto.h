...
public:
    ...
    void OnInfoRequest(CHARGUID charGuid, MSG_BASE* packet) { currentState_->OnInfoRequest(charGuid, packet); }
    // [IN] MSG_CW_LOTTO_ENTRY_SYN
    void OnEntryRequest(CHARGUID charGuid, MSG_BASE* packet) { currentState_->OnEntryRequest(charGuid, packet); }
    // [IN] MSG_CW_LOTTO_NOTICE_LIST_SYN
    void OnNoticeRequest(CHARGUID charGuid, MSG_BASE* packet) { currentState_->OnNoticeRequest(charGuid, packet); }
    // [IN] MSG_CW_LOTTO_NOTICE_CONFIRM
    void OnNoticeConfirm(CHARGUID charGuid, MSG_BASE* packet) { currentState_->OnNoticeConfirm(charGuid, packet); }

    // 서버 수신 패킷 핸들러
    //
    // [IN] MSG_DW_LOTTO_LOAD_ENTRY_ACK
    void OnEntryLoad(MSG_BASE* packet) { currentState_->OnEntryLoad(packet); }
    // [IN] MSG_DW_LOTTO_LOAD_ACK
    void OnLoad(MSG_BASE* packet) { currentState_->OnLoad(packet); }
    // [IN] MSG_DW_LOTTO_SAVE_ENTRY_ACK
    void OnEntrySave(MSG_BASE* packet) { currentState_->OnEntrySave(packet); }
    // [IN] MSG_DW_LOTTO_WINNER_LIST_ACK
    void OnWinnerList(MSG_BASE* packet) { currentState_->OnWinnerList(packet); }
    // [IN] MSG_DW_LOTTO_ROLLOVER_ACK
    void OnRollOver(MSG_BASE* packet) { currentState_->OnRollOver(packet); }
    // [IN] MSG_DW_LOTTO_ADD_TICKET_ACK
    void OnTicketAcquisition(MSG_BASE* packet) { currentState_->OnTicketAcquisition(packet); }
    // [IN] MSG_DW_LOTTO_REWARD_ACK
    void OnReward(MSG_BASE* packet) { currentState_->OnReward(packet); }
    // [IN] MSG_DW_LOTTO_NOTICE_LIST_ACK
    void OnNoticeList(MSG_BASE* packet) { currentState_->OnNoticeList(packet); }
    // [IN] MSG_DW_LOTTO_NOTICE_DEL_ACK
    void OnNoticeDelete(MSG_BASE* packet) { currentState_->OnNoticeDelete(packet); }
    // [IN] MSG_AW_USER_STATUS_HEIM_CONSUMPTION_NTF
    void OnHeimConsumption(MSG_BASE* packet) { currentState_->OnHeimConsumption(packet); }
    // [IN] MSG_AW_USER_LOTTO_REWARD_ACK
    void OnActualReward(MSG_BASE* packet) { currentState_->OnActualReward(packet); }
#ifdef _NA_0_20100115_HEIM_LOTTO_TICKET_PURCHASE
    // [IN] MSG_AW_USER_LOTTO_TICKET_PURCHASE_ACK
    void OnTicketPurchase(MSG_BASE* packet) { currentState_->OnTicketPurchase(packet); }
#endif // _NA_0_20100115_HEIM_LOTTO_TICKET_PURCHASE
#ifdef _NA_20101011_HEIM_LOTTO_CASH_TICKET
    // [IN] MSG_AW_ITEM_USE_HEIM_LOTTO_TICKET_SYN
    void OnTicketActivation(MSG_BASE* packet) { currentState_->OnTicketActivation(packet); }
#endif //
...
private:
    ...
    // 응모 내역을 추가한다
    void EnterTicket(CHARGUID charGuid, int drawNum, int status, BYTE* pickedNum);
