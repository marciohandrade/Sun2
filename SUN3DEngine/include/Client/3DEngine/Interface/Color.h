...
/*!
 CColor: Repr?entiert einen Farbwert
 */
class CColor
{
public:
...
private:
  ...
  enum {c_red = 0, c_green = 1, c_blue = 2, c_null = 3}; // enum Hack f? colorbyte-Index

  float m_hue;         // 0.0 .. 360.0  // Winkel
  float m_saturation;  // 0.0 .. 1.0    // Prozent
  float m_luminance;   // 0.0 .. 1.0    // Prozent

  // Flag f? Lazy Evaluation
  bool m_bIsRGB; 
  bool m_bIsHLS; 

  // statische Konstanten f? benannte Farben
  struct DNamedColor
  {
    COLORREF color; 
    LPCTSTR  name;
