...
SonnenScheinScene g_SonnenScheinScene;

SonnenScheinScene::SonnenScheinScene()
{
	m_SceneType = SCENE_TYPE_SONNENSCHEIN;
}
...
BOOL SonnenScheinScene::Process( DWORD dwTick )
{
	BOOL bReturn;
...
void SonnenScheinScene::Release()
{
	FieldScene::Release();
...
void SonnenScheinScene::ParsePacket( MSG_BASE * pMsg )
{
	FieldScene::ParsePacket(pMsg);
}

RELATIONKIND::TYPE SonnenScheinScene::GetRelationKindOfCharacter( Character* pObject, BOOL bForceEnemy )
{
	return GameFunc::GetRelationKindOfPlayerInDominationWar(pObject, bForceEnemy);
}

#ifdef _NA_007115_20140313_MONDSCHIEN_RENEWAL
BOOL SonnenScheinScene::PostInterfaceRender( DWORD tick )
{
