...
#include "uiTargetZoneMarkDialog.h"

//------------------------------------------------------------------------------
 WzID uiTargetZoneMarkDialog::constols_id_[DIALOG_MAX] = 
 {
...
/**
*/
uiTargetZoneMarkDialog::uiTargetZoneMarkDialog(InterfaceManager *pUIMan)
:uiBase(pUIMan)
{
#ifdef _NA_000000_20130114_RENEWER_UI
...
/**
*/
uiTargetZoneMarkDialog::~uiTargetZoneMarkDialog()
{
...
*/
#ifdef _NA_000000_20130114_RENEWER_UI
VOID uiTargetZoneMarkDialog::Release()
{
...
void uiTargetZoneMarkDialog::Clear()
{
...
/**
*/
void uiTargetZoneMarkDialog::Init(CDrawBase* pDrawBase)
{
	...
	CCtrlPictureWZ* zonemark_picture = static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PICl00]));
	zonemark_picture->ShowWindowWZ(WZ_SHOW);

	//최대치의 존마크는 99개로 포함시킨다.
	max_count_zonemark_ = 99;

#else
	for (int Index = 0; Index < MAX_ZONEMARK; ++Index)
	{
		zonemark_picture_[Index] = static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC1+Index]));
        if (zonemark_picture_[Index] != NULL)
        {
            ++max_count_zonemark_;
        }
	}
...
/**
*/
void uiTargetZoneMarkDialog::OnShowWindow(BOOL val)
{
	if (val)
	{
        RECT disable_rect;
        MoveDefaultWindowPos();
        GetDialogWZ()->GetClientRect(&disable_rect);
        DisableInterSectRect(disable_rect);

        if (0 == player_zonemark_reward_info_.GetRewardCount()
#ifdef _NA_008334_20150608_SONNENSCHEIN
            && (sonnenschein_level_ == 0)
#endif //_NA_008334_20150608_SONNENSCHEIN
            )
...
/**
*/
void uiTargetZoneMarkDialog::OnRenderSolarDialog()
{
	//마우스 오버시 툴팁 출력
	...
	{
        if (zonemark_picture_[RewardIndex] == NULL)
        {
            continue;
        }

		rc = zonemark_picture_[RewardIndex]->GetSizeRect();
		if(iMouseX > rc.left && 
           iMouseX < rc.right && 
           iMouseY > rc.top && 
           iMouseY < rc.bottom)
    ...
    {
        zonemark_picture = static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC003]));
        if(zonemark_picture && tooltip_manager)
        {
            RECT rc = zonemark_picture->GetSizeRect();
            if(iMouseX > rc.left && 
...
*/
#ifdef _NA_008334_20150608_SONNENSCHEIN
void uiTargetZoneMarkDialog::SetZoneMarkInfo(STLX_VECTOR<MAPCODE>& domination_state_info, BYTE sonnenschein_level)
#else
void uiTargetZoneMarkDialog::SetZoneMarkInfo(STLX_VECTOR<MAPCODE>& domination_state_info)
#endif //_NA_008334_20150608_SONNENSCHEIN
{
	//예기서 갯수 증가

#ifdef _NA_008334_20150608_SONNENSCHEIN
    player_zonemark_reward_info_.SetZoneMarkInfo(domination_state_info, sonnenschein_level);
#else
    ...
    for (DWORD index = 0; index < max_count_zonemark_; ++index)
    {
        ...
        {
            zonemark_picture_[index]->ShowWindowWZ(WZ_SHOW);
        }
        else
        {
            zonemark_picture_[index]->ShowWindowWZ(WZ_HIDE);
        }
    }
#endif
}
...
#ifdef _NA_000000_20130114_RENEWER_UI
void uiTargetZoneMarkDialog::returnImageIndextoRect( int mark_count,int* score_index )
{
...
int uiTargetZoneMarkDialog::CheckModifyReturnValue( int mark_count,int* array_count )
{
	int iDigitcount = 0;
...
void uiTargetZoneMarkDialog::render_mark_count_Image()
{
	...
	//시작 위치값
	CCtrlPictureWZ* text_picture1= static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC000]));
	...
	for(int i = 1; i >= 0; --i)
	{
		text_picture1= static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC001-i]));
    ...
    {
        text_picture1= static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC002]));
        if (text_picture1)
        {
            // " - " 출력
            rect = text_picture1->GetSizeRect();
        ...
        text_picture1= static_cast<CCtrlPictureWZ*>(GetControlWZ(constols_id_[DLGPOS_ZONEMARK_PIC003]));
        if (text_picture1)
        {
            // 단계값 출력
            rect = text_picture1->GetSizeRect();
...
void uiTargetZoneMarkDialog::option_domination_calu()
{
...
void uiTargetZoneMarkDialog::OnUpdateSolarDialog()
{
