...
// elsewhere.  
    ...
    // Added as a fix for the stack overflow. 
	// For small strings the memory is allocated on stack, for large - in heap 
	class _CSmartPtr
	{
	public:
		void * m_ptr;
		_CSmartPtr() : m_ptr(NULL) {}
		~_CSmartPtr() 
		{ 
			if( NULL != m_ptr ) 
			{ 
		...
		{ 
			...
			#define SSCVT int _cvt; _cvt; UINT _acp=GetACP(); \
			    _acp; PCWSTR _pw; _pw; PCSTR _pa; _pa; _CSmartPtr _ptr 
		#else
			#define SSCVT int _cvt = 0; _cvt; UINT _acp=GetACP();\
			    _acp; PCWSTR _pw=0; _pw; PCSTR _pa=0; _pa; _CSmartPtr _ptr
		#endif
	#else
		#ifndef _DEBUG
			#define SSCVT int _cvt; _cvt; UINT _acp=CP_ACP; _acp;\
			     PCWSTR _pw; _pw; PCSTR _pa; _pa; _CSmartPtr _ptr
		#else
	...
	// Added as a fix for the stack overflow. It is important that the "sizeExp" will be constant! 
	// For small strings the memory is allocated on stack, for large - in heap 
	#define _ALLOC128(sizeExp) ( (sizeExp) > 127 ? _ptr.alloc(sizeExp) : _alloca(sizeExp))
	
	#ifdef _CONVERSION_USES_THREAD_LOCALE
		#define SSA2W(pa) (\
			((_pa = pa) == 0) ? 0 : (\
			_cvt = (strlen(_pa)+1),\
			StdCodeCvt((PWSTR) _ALLOC128(_cvt*2), _pa, _cvt, _acp)))
		#define SSW2A(pw) (\
			((_pw = pw) == 0) ? 0 : (\
			_cvt = (wcslen(_pw)+1)*2,\
			StdW2AHelper((LPSTR) _ALLOC128(_cvt), _pw, _cvt, _acp)))
	#else
		#define SSA2W(pa) (\
				((_pa = pa) == 0) ? 0 : (\
				_cvt = (strlen(_pa)+1),\
				StdCodeCvt((PWSTR) _ALLOC128(_cvt*2), _pa, _cvt)))
		#define SSW2A(pw) (\
				((_pw = pw) == 0) ? 0 : (\
				_cvt = (wcslen(_pw)+1)*2,\
				StdCodeCvt((LPSTR) _ALLOC128(_cvt), _pw, _cvt)))
    #endif
...
//	std::collate<CT>::string_type s2(sz2);
    std::basic_string<CT> s1(sz1 ? sz1 : "");
    std::basic_string<CT> s2(sz2 ? sz2 : "");
                ...
                const A12& v12, const A13& v13, const A14& v14)
    {
		...
		if ( strFmt.Load(nId) )
        {
            Fmt(strFmt, FmtArg<A1>(v1).Val(), FmtArg<A2>(v2).Val(),
                ...
                FmtArg<A6>(v6).Val(), FmtArg<A7>(v7).Val(), FmtArg<A8>(v8).Val(),
                FmtArg<A9>(v9).Val(), FmtArg<A10>(v10).Val(),FmtArg<A11>(v11).Val(),
                FmtArg<A13>(v12).Val(), FmtArg<A13>(v13).Val(),FmtArg<A14>(v14).Val());
        }
    }
    template<class A1, class A2, class A3, class A4, class A5, class A6,
        class A7, class A8, class A9, class A10, class A11, class A12,
