...
bool CMarkupArchive::Open(LPCTSTR szFileName)
{
	...
	else
	{
		...
		#else
			gzFile gzf = gzopen( m_sFileName.c_str(), "rb");
		#endif
...
//	str.Format(_T("<?xml version=\"1.0\" encoding=\"%s\" ?>\r\n"), m_strEncodingString);
	...
	if (IsStoring())
	{
		if (!( m_dwMode & ArchiveModeNotZipped))
		{
			...
			// --- end --- TMB! 29-08-2002 ** INSAD **
			length=str.size()*sizeof(TCHAR);

		//	pBuffer=str.LockBuffer();
			pBuffer=(char*)str.c_str();
		...
		else
		{
...

			{
				...
				hFile = CreateFile(m_sFileName.c_str(),           // create MYFILE.TXT 
...
bool CMarkupArchive::FindGetChildData(LPCTSTR szName, INT64& _varint64)
{
	bool found;

	ResetChildPos();
	if  (found = FindChildElem(szName))
		_varint64 = GetChildDataInt64();
	return found;
};
...
bool CMarkupArchive::FindGetChildData(LPCTSTR szName, SHORT& _short)
{
	bool found;

	ResetChildPos();
	if  (found = FindChildElem(szName))
		_short = GetChildDataShort();
	return found;
};

bool CMarkupArchive::FindGetChildData(LPCTSTR szName, char* _tchar)
{
	bool found;

	ResetChildPos();
	if  (found = FindChildElem(szName))
		_tchar = const_cast<char*>(GetChildDatachar());
	return found;
};
