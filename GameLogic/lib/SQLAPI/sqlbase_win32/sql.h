/* COPYRIGHT (C) GUPTA CORPORATION 1984-1998 */
/*
INTERFACE TO
  SQL
*/
...
/* VERSION NUMBER */
#define   SQLVERS   70001		/* version number		    */
#define   SQLAPIV   00102		/* API version number since 3.6     */
/* DEFINED CONSTANTS */

/*-------------------------------------------------------------------------
  Define two C macros that specify whether the target object files will
  be 16-bit or 32-bit.	The default is 16-bit object code.
  ------------------------------------------------------------------------- */

#ifndef SQL_32BITTARG
# define SQL_32BITTARG	 0		/* 32-bit target object files	    */
#endif

#ifndef SQL_16BITTARG
# if !SQL_32BITTARG
#  define SQL_16BITTARG  1		/* 16-bit target object files	    */
# else
#  define SQL_16BITTARG  0
# endif
#endif

#if SQL_16BITTARG && SQL_32BITTARG	/* can't have both be true	    */
#  error SQL_16BITTARG and SQL_32BITTARG cannot both be true
#endif

#if !SQL_16BITTARG && !SQL_32BITTARG	/* one must be true		    */
#  error Either SQL_16BITTARG and or SQL_32BITTARG must be true
#endif
...
# define PTR *
#else
# define   PTR		  far * 	/* 16-bit far pointer		    */
#endif					/* end if SQL_32BITTARG 	    */

/*-------------------------------------------------------------------------
  To build a 32-bit OS/2 program, SQL_OS2 must be set to 1.  (Do NOT
  set this if building a 16-bit OS/2 program.)	The SQL_OS2 define
  is used below to determine the calling convention for the SQL API.
  If your OS/2 compiler supports the predefined macro __OS2__
  (e.g., Borland or IBM),  then you do not need to do anything.
  If you are using a compiler that does not define this macro
  (e.g., the Watcom DOS-hosted compiler building an OS/2 target
  object), then you must define SQL_OS2 to be 1 before #including SQL.H.
  ------------------------------------------------------------------------- */
...
#if SQL_OS2 && SQL_16BITTARG		/* cannot both be true		    */
#  error SQL_OS2 is for 32-bit programs only; do not use with SQL_16BITTARG
#endif
...
#define   SQLPDTM   24			/* data type maximum		    */
/* EXTERNAL DATA TYPES */
...
#define   SQLMCMT   104 		/* max command types		    */
#define   SQLMCNM   18			/* max referential constraint name  */
...
#if SQL_16BITTARG			/* 16-bit			    */
   typedef int (far pascal *SQLTPFP)(void);
#elif SQL_OS2				/* 32-bit OS/2			    */
   typedef int (*SQLTPFP)(void);
#else					/* 32-bit non-OS/2		    */
...
#define NUMBER_IS_NULL(x)     (x.datetime_len == 0)
#define NUMBER_SET_NULL(x)    (x.datetime_len = 0)
...
/*-------------------------------------------------------------------------
  ...
  For 16-bit systems, the pascal calling convention is used.
  ...
  - For OS/2 2.x, the "system" calling convention is used; this is called
    _System by the IBM compiler and __syscall by the Borland and Watcom
    compilers.
  ------------------------------------------------------------------------- */

#if SQL_16BITTARG			/* 16-bit all use pascal	    */
#  define SQLTAPI byte2 far pascal
#elif SQL_OS2
#  ifdef __IBMC__			/* 32-bit IBM OS/2 C uses _System... */
#    define SQLTAPI byte2 _System
#  else 				/* ...other OS/2 C uses __syscall   */
#    define SQLTAPI byte2 __syscall
#  endif
#else
#  define SQLTAPI byte2 SBSTDCALL	/* 32-bit non-OS/2 uses __stdcall   */
#endif					/* end if SQL_16BITTARG 	    */
...
#define SQLFMAXAPI  SQLFRSI		/* points change		    */
